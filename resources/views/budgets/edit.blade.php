            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6">
                <form action="{{ route('budgets.update', $budget->id) }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" name="year" id="year" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" value="{{ old('year', $budget->year) }}" required>
                        @error('year')
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" name="month" id="month" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" value="{{ old('month', $budget->month) }}" min="1" max="12">
                        @error('month')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    <div class="mb-4">
                        ...
                        <input type="number" step="0.01" name="amount" id="amount" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" value="{{ old('amount', $budget->amount) }}" required>
                        @error('amount')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    <div class="mb-4">
                        ...
                        <input type="text" name="comment" id="comment" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" value="{{ old('comment', $budget->comment) }}">
                        @error('comment')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
